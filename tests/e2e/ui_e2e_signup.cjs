...
(async () => {
  ...
  page.on('request', req => {
    try {
      if (req.url().includes('/api/')) {
        const body = req.postData ? (req.postData().substring(0,400)) : '';
        console.log('[E2E] REQ', req.method(), req.url(), body ? ('BODY:'+body) : '');
      }
    } catch (e) {
      console.log('[E2E] REQ_ERR', e.message);
    }
  });
  ...
  try {
    ...
    // Debug: list buttons text
    const btns = await page.evaluate(() => Array.from(document.querySelectorAll('button')).map(b => b.innerText && b.innerText.trim()));
    out('BUTTON_TEXTS: ' + JSON.stringify(btns));

    // Click Continuar (find by button text)
    const clicked = await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText && b.innerText.toLowerCase().includes('continuar'));
      if (btn) { btn.click(); return true; }
      return false;
    });
    if (clicked) out('Clicked Continuar (to step 1)'); else throw new Error('Continuar button not found');
    ...
    if (cont2) { out('Clicked Continuar (to step 2)'); await new Promise(r => setTimeout(r, 500)); }
    ...
    // Debug: list buttons before final submit
    const btns2 = await page.evaluate(() => Array.from(document.querySelectorAll('button')).map(b => b.innerText && b.innerText.trim()));
    out('BUTTON_TEXTS_STEP2: ' + JSON.stringify(btns2));

    // Click Criar Negócio (case-insensitive)
    const created = await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll('button')).find(b => b.innerText && (b.innerText.toLowerCase().includes('criar') || b.innerText.toLowerCase().includes('cadastrar')));
      if (btn) { btn.click(); return true; }
      return false;
    });
    if (created) out('Clicked Criar Negócio'); else throw new Error('Criar Negócio button not found');
