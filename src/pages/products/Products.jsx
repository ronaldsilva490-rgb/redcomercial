...
export default function Products() {
  ...
  const handleSave = async () => {
    ...
    if (!form.preco_venda && form.preco_venda !== 0) return toast.error('Tudo tem seu preço! Conta pra gente quanto isso custa 💵')
    setSaving(true)
    try {
      const payload = {
        ...form,
      ...
      if (modal === 'new') {
        ...
        toast.success('Prontinho! Detalhes do produto foram salvos 🏷️')
      }
      close(); load()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.delete(`/api/products/${id}`); toast.success('Item apagado com sucesso! 🗑️'); load() }
