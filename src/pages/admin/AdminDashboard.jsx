...
import {
  ...
  Blocks, ToggleLeft, ToggleRight, Settings, Save, MessageCircle
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (adminUser) setAdmin(JSON.parse(adminUser))
    setCarregando(false)
  }, [])
  ...
  const carregarDados = useCallback(async () => {
    setAtualizando(true)
    try {
      const [statusRes, tenantsRes, adminsRes] = await Promise.all([
        api.get('/api/admin/status').catch(() => null),
        api.get('/api/admin/tenants').catch(() => null),
        api.get('/api/admin/list').catch(() => null),
      ])
  ...
  useEffect(() => { if (aba === 'logs' && !viewTerminal) carregarLogs() }, [logFiltro, aba, viewTerminal, carregarLogs])

  const carregarLogsFly = useCallback(async () => {
    try {
      const res = await api.get('/api/admin/fly-logs')
      if (res?.data?.data) {
        setFlyLogs(res.data.data)
      }
    } catch (err) {
      console.error('Erro ao carregar logs do Fly:', err)
    }
  }, [])

  useEffect(() => {
    if (aba === 'logs' && viewTerminal) {
      carregarLogsFly()
      const interval = setInterval(carregarLogsFly, 3000)
      return () => clearInterval(interval)
    }
  }, [aba, viewTerminal, carregarLogsFly])

  const fetchNetworkInfo = useCallback(async () => {
    setNetworkLoading(true)
    try {
      const res = await api.get('/api/admin/network-info')
  ...
  const handleLogout = () => {
    // Limpa TODOS os tokens (admin + supabase) para evitar sessões zumbi
  ...
  const desativarAdmin = async (id) => {
    ...
    try {
      await api.post(`/api/admin/deactivate/${id}`, {})
      toast.success('Administrador temporariamente na geladeira 🧊')
      carregarDados()
    } catch { toast.error('Deu um errinho ao tentar desativar... 😕') }
  }
  ...
  const deletarAdmin = async (id) => {
    ...
    try {
      ...
      toast.success('Conta de admin apagada definitivamente! 🗑️')
      carregarDados()
    } catch { toast.error('Puxa, não consegui apagar o registro agora 😕') }
  }


  const handleToggleModulo = (modId) => {
    if (!modalTenant) return
    const cfg = modalTenant.config || {}
    const ativos = cfg.modulos_ativos || []
    const isAtivo = ativos.includes(modId)
    const newAtivos = isAtivo ? ativos.filter(a => a !== modId) : [...ativos, modId]
    setModalTenant({ ...modalTenant, config: { ...cfg, modulos_ativos: newAtivos } })
  }

  const handleSalvarModulos = async () => {
    setSalvandoConfig(true)
    try {
      await api.patch(`/api/admin/tenants/${modalTenant.id}`, { config: modalTenant.config })
      toast.success('Configurações da Empresa salvas com sucesso!')
      setModalTenant(null)
      carregarDados()
    } catch {
      toast.error('Opa, falha ao salvar configurações')
    } finally {
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 20px', position: 'relative', zIndex: 1 }}>
                ...
                {tenants.map(t => (
                  <div key={t.id} style={{ ...CARD, padding: '16px 18px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                      ...
                      <button onClick={() => setModalTenant(t)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', cursor: 'pointer', transition: 'color 0.2s', padding: 4 }} title="Configurar Empresa" onMouseEnter={e => e.currentTarget.style.color = '#fff'} onMouseLeave={e => e.currentTarget.style.color = 'rgba(255,255,255,0.4)'}>
                        <Settings size={16} />
                      </button>
                    </div>
                    ...
                    {t.criado_em && (
                      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.25)', marginTop: 8, borderTop: '1px solid rgba(255,255,255,0.04)', paddingTop: 8, display: 'flex', justifyContent: 'space-between' }}>
                        <span>Criado em {new Date(t.criado_em).toLocaleDateString('pt-BR')}</span>
                      </div>
                    )}
                  </div>
                ))}
        ...
        {/* ══ LOGS (DATABASE + TOGGLE TERMINAL) ══ */}
        {aba === 'logs' && (
          <div className="fade-in">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{ fontSize: 13, fontWeight: 700 }}>Logs do Sistema ({viewTerminal ? 'Live' : logTotal})</div>
                <button
                  onClick={() => setViewTerminal(!viewTerminal)}
                  style={{
                    padding: '4px 10px', fontSize: 10, fontWeight: 700, borderRadius: 6, cursor: 'pointer',
                    background: viewTerminal ? 'rgba(34,197,94,0.1)' : 'rgba(255,255,255,0.05)',
                    color: viewTerminal ? '#22c55e' : 'rgba(255,255,255,0.4)',
                    border: `1px solid ${viewTerminal ? 'rgba(34,197,94,0.3)' : 'rgba(255,255,255,0.1)'}`,
                  }}
                >
                  {viewTerminal ? '⚡ MODO LIVE ON' : '📡 ATIVAR LIVE (FLY)'}
                </button>
              </div>
              <button
                onClick={async () => {
                  if (window.confirm('Wipe total nos logs do banco e limpar tela. Confirma?')) {
                    try {
            ...
            {viewTerminal && (
              <div className="fade-in">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', background: 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: 4 }}>
                    Terminal Polling: 3s
                  </span>
                  <button
                    ...
                    style={{
                      ...
                      border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, color: 'rgba(255,255,255,0.5)', cursor: 'pointer'
                    }}
                  >
                ...
                <div style={{
                  ...
                  borderRadius: 14, padding: '16px', height: '55vh', overflowY: 'auto',
                  fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', position: 'relative'
                }}>
                  <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'repeating-linear-gradient(0deg, rgba(0,0,0,0.15), rgba(0,0,0,0.15) 1px, transparent 1px, transparent 2px)', zIndex: 2 }} />
                  {flyLogs.length === 0 ? (
                    <div style={{ color: 'rgba(255,255,255,0.2)', fontStyle: 'italic' }}>Aguardando fluxo...</div>
                  ) : (
                    <div style={{ position: 'relative', zIndex: 1 }}>
                      {flyLogs.map((log, i) => (
                        <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 4 }}>
                          <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: '10px' }}>[{log.timestamp ? new Date(log.timestamp).toLocaleTimeString() : '??'}]</span>
                          <span style={{ color: log.level?.toLowerCase().includes('err') ? '#ff4d4d' : '#4ade80', fontWeight: 700, width: 40, fontSize: '10px' }}>{log.level || 'INFO'}</span>
                          <span style={{ color: '#fff', wordBreak: 'break-all' }}>{log.message}</span>
                        </div>
                      ))}
                      <div ref={el => { if (el && autoScroll) el.scrollIntoView({ behavior: 'smooth' }) }} />
                    </div>
                  )}
                </div>
              </div>
      ...
      {modalTenant && (
        <div style={{
          ...
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          zIndex: 1000, padding: 20, backdropFilter: 'blur(5px)'
        }}>
          <div style={{
            ...
            <button
              ...
              style={{ position: 'absolute', top: 20, right: 20, background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', cursor: 'pointer' }}
            >
              <XCircle size={20} />
            </button>
            <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 8 }}>
              <Settings size={18} color="#c41217" /> Configurar Empresa
            </div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', marginBottom: 24 }}>
              Gerenciando Módulos de: <strong style={{color: '#fff'}}>{modalTenant.nome} ({modalTenant.slug})</strong>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                { id: 'mesas', label: 'Mesas & Comandas', desc: 'Habilita o mapa de mesas e atendimento de salão via garçons' },
                { id: 'cozinha', label: 'Monitor de Cozinha (KDS)', desc: 'Habilita a tela exclusiva para os cozinheiros prepararem pratos' },
                { id: 'delivery', label: 'Módulo de Entregas (Delivery)', desc: 'Habilita a área visual para despacho de pedidos e entregadores' },
              ].map(mod => {
                const ativos = modalTenant.config?.modulos_ativos || []
                const isAtivo = ativos.includes(mod.id)
                return (
                  <div key={mod.id} onClick={() => handleToggleModulo(mod.id)} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    padding: '14px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 12,
                    border: isAtivo ? '1px solid #c41217' : '1px solid rgba(255,255,255,0.08)',
                    cursor: 'pointer', transition: 'all 0.2s'
                  }}>
                    <div>
                      <div style={{ fontSize: 13, fontWeight: 600, color: isAtivo ? '#fff' : 'rgba(255,255,255,0.5)' }}>
                        {mod.label}
                      </div>
                      <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>{mod.desc}</div>
                    </div>
                    {isAtivo ? <ToggleRight size={24} color="#c41217" /> : <ToggleLeft size={24} color="rgba(255,255,255,0.2)" />}
                  </div>
                )
              })}
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
              <button
                ...
                style={{
                  ...BTN_RED, flex: 1, padding: '12px', fontSize: 13,
                  opacity: salvandoConfig ? 0.6 : 1, display: 'flex', justifyContent: 'center', gap: 8
                }}
              >
                {salvandoConfig ? <RefreshCw size={14} className="spin" /> : <Save size={14} />}
                {salvandoConfig ? 'Salvando...' : 'Salvar Módulos'}
              </button>
            </div>
          </div>
        </div>
