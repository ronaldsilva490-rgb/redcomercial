...
export default function Finance() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast.error("Oops! Não consegui buscar financeiro. Tente novamente 😕") }
  ...
  const handleSave = async () => {
    ...
    if (!form.data_vencimento) return toast.error('Sem data não rola... Quando essa conta vence? 🗓️')
    setSaving(true)
    try {
      const payload = {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      toast.success('Dinheiro no caixa! Conta paga com sucesso 💲')
      close(); load()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      toast.success('Item apagado com sucesso! 🗑️')
      load()
    } catch (err) { toast.error(err.response?.data?.error || 'Erro') }
  }
