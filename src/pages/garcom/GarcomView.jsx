...
export default function GarcomView() {
  ...
  const handleSendOrder = async () => {
    if (!cart.length) return toast.error('Adicione itens ao carrinho primeiro')
    if (!activeOrder) return toast.error('Nenhuma comanda aberta')
    setSending(true)
    try {
      ...
      setCart([])
      toast.success('Pedido enviado! 🍽️')
    } catch (e) {
  ...
  const handleSolicitarPagamento = async () => {
    ...
    try {
      ...
      toast.success('Caixa notificado! 🔔')
      setPayStep(null)
