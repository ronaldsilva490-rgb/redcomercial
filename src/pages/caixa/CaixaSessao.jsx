...
export default function CaixaSessao({ embedded = false }) {
  ...
  const handleAbrir = async () => {
    ...
    try {
      ...
      toast.success('Caixa aberto e brilhando! Vamo que vamo 🚀')
      setAbrirModal(false)
      setFundo('')
      setObsAbrir('')
      await load()
  ...
  const handleFechar = async () => {
    ...
    try {
      ...
      toast.success('Caixa trancado! Descanso merecido 👋')
  ...
  return (
    <div>
      ...
      {/* Header */}
      {embedded ? (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 20, borderBottom: '1px solid var(--border)', paddingBottom: 16 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text)' }}>💰 Gestão de Caixa</div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {sessao && isManager && (
              <button className="btn btn-danger btn-sm" onClick={() => setFecharModal(true)}>
                <Lock size={14} /> Fechar Caixa
              </button>
            )}
            {!sessao && isManager && (
              <button className="btn btn-primary btn-sm" onClick={() => setAbrirModal(true)}>
      ...
      {sessao ? (
        ...
        <FechadoCard ultimaSessao={ultimaSessao} onAbrir={!embedded && isManager ? () => setAbrirModal(true) : null} />
      )}
